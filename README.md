...
1. `config/services.yaml` заменён на `config/services.php` с импортами любых `di.php` из `src`.
1. `config/routes.yaml` заменён на `config/routes.php` с импортами любых `routing.php` из `src`.
1. `Kernel` перенесён в `src/Infrastructure`.
...
1. Модуль в `src` — это бизнесовый или инфраструктурный код, бандл — только инфраструктурный. Как правило, бандл — это бридж между какой-нибудь библиотекой и Symfony. Бандлы решают технические задачи и не имеют отношения к предметной области конкретного проекта.
